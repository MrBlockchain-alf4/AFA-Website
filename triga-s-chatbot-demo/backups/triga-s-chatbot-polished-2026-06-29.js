...
/* ─── Calendar body ── */
.tgs-bk-body{flex:1;overflow:hidden;padding:12px 12px 10px;display:flex;flex-direction:column;}
.tgs-bk-grid{display:grid;grid-template-columns:repeat(5,1fr);grid-template-rows:auto;grid-auto-rows:1fr;gap:6px;flex:1;}
.tgs-bk-dow{
...
/* ─── Time slots area ── */
.tgs-bk-slots-area{flex:1;overflow:hidden;padding:12px 12px 10px;}
.tgs-bk-slot-grid{
...
/* ─── Confirm body ── */
.tgs-bk-conf-body{flex:1;overflow:hidden;padding:10px 13px 6px;}
.tgs-bk-conf-section{margin-bottom:9px;}
...
.tgs-bk-anl-toggle{
  background:none;border:none;cursor:pointer;
  font-size:10px;color:#3A80C4;font-weight:500;
...
.tgs-bk-conf-sec-row .tgs-bk-btn.sec{font-size:10px;height:30px;}
  ...
  function showConfirm(mode){
    ...
    setTimeout(()=>{
      ...
      function mkAnliegenRow(interest){
        const r=document.createElement('div'); r.className='tgs-bk-conf-row';
        const l=document.createElement('span'); l.className='tgs-bk-sec-lbl'; l.textContent='Thema';
        if(!interest){
          const v=document.createElement('span'); v.className='tgs-bk-sec-val tgs-bk-sec-empty'; v.textContent='Nicht angegeben';
          r.appendChild(l); r.appendChild(v); return r;
        }
        const maxLen=120;
        const truncated=interest.length>maxLen;
        const wrap=document.createElement('span'); wrap.style.cssText='flex:1;text-align:right;display:flex;flex-direction:column;align-items:flex-end;';
        const v=document.createElement('span'); v.className='tgs-bk-sec-val'; v.textContent=truncated?interest.slice(0,maxLen)+'…':interest;
        wrap.appendChild(v);
        if(truncated){
        ...
        r.appendChild(l); r.appendChild(wrap); return r;
      }

      const apptSec=mkSection('Termin');
      apptSec.appendChild(mkRow('Datum',fmtDate(S.cal.selDate)));
      apptSec.appendChild(mkRow('Uhrzeit',S.cal.selTime+' Uhr'));
      body.appendChild(apptSec);

      if(mode==='booking'){
        const contSec=mkSection('Kontaktdaten');
        contSec.appendChild(mkRow('Vorname',S.data.vorname?esc(S.data.vorname):'Nicht angegeben',!S.data.vorname));
        contSec.appendChild(mkRow('Nachname',S.data.nachname?esc(S.data.nachname):'Nicht angegeben',!S.data.nachname));
        contSec.appendChild(mkRow('E-Mail',S.data.email?esc(S.data.email):'Nicht angegeben',!S.data.email));
        contSec.appendChild(mkRow('Telefon',S.data.telefon?esc(S.data.telefon):'Nicht angegeben',!S.data.telefon));
        contSec.appendChild(mkRow('Firma',S.data.unternehmen?esc(S.data.unternehmen):'Nicht angegeben',!S.data.unternehmen));
        body.appendChild(contSec);
        const anlSec=mkSection('Anliegen');
        anlSec.appendChild(mkAnliegenRow(S.data.interesse));
        body.appendChild(anlSec);
      }
      if(mode==='reschedule'){
        const reschedSec=mkSection('Aktueller Termin');
        reschedSec.appendChild(mkRow('Termin',esc(S.appointment?.termin||'–')));
        body.appendChild(reschedSec);
      }
