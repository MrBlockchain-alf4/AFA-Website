  ...
  function showConfirm(mode){
    ...
    setTimeout(()=>{
      ...
      if(mode==='booking'){
        ...
        contSec.appendChild(mkRow('Vorname',vn?esc(vn):'Nicht angegeben',!vn));
        contSec.appendChild(mkRow('Nachname',nn?esc(nn):'Nicht angegeben',!nn));
        contSec.appendChild(mkRow('E-Mail',em?esc(em):'Nicht angegeben',!em));
        contSec.appendChild(mkRow('Telefon',tel?esc(tel):'Nicht angegeben',!tel));
        contSec.appendChild(mkRow('Firma / Org.',fi?esc(fi):'Nicht angegeben',!fi));
        body.appendChild(contSec);
        const anlSec=mkSection('Anliegen');
        anlSec.appendChild(mkAnliegenRow(int));
        if(msg) anlSec.appendChild(mkRow('Nachricht',msg.length>100?msg.slice(0,100)+'…':esc(msg)));
