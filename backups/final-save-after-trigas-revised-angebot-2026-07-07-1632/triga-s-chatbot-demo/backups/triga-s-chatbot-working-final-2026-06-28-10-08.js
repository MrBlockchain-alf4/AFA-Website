...
/* ─── CALENDAR ───────────────────────────────────── */
.tgs-cal{background:#fff;border-radius:14px;border:1px solid #D4E4F0;
  overflow:hidden;box-shadow:0 3px 14px rgba(0,59,112,.08);}
.tgs-cal-hd{background:#003B70;padding:10px 14px;
  display:flex;align-items:center;justify-content:space-between;}
.tgs-cal-nav{width:28px;height:28px;border-radius:50%;
  background:rgba(255,255,255,.12);border:none;cursor:pointer;color:#fff;
  font-size:16px;display:flex;align-items:center;justify-content:center;
  transition:background .13s;line-height:1;}
.tgs-cal-nav:hover:not(:disabled){background:rgba(255,255,255,.22);}
.tgs-cal-nav:disabled{opacity:.2;cursor:not-allowed;}
.tgs-cal-mo{color:#fff;font-size:13.5px;font-weight:700;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}
.tgs-cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:2px;padding:8px 8px 6px;}
.tgs-cal-dow{font-size:9px;font-weight:700;color:#B0C2D2;text-align:center;
  padding:2px 0 5px;text-transform:uppercase;letter-spacing:.06em;}
.tgs-cal-day{aspect-ratio:1;border-radius:50%;border:none;background:transparent;
  cursor:pointer;font-size:12px;font-weight:500;color:#1A2636;
  display:flex;align-items:center;justify-content:center;
  transition:background .12s,color .12s;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}
.tgs-cal-day:hover:not(:disabled):not(.tgs-sel){background:#E0EEF8;color:#003B70;}
.tgs-cal-day.tgs-today{color:#003B70;font-weight:800;}
.tgs-cal-day.tgs-sel{background:#003B70;color:#fff;font-weight:700;}
.tgs-cal-day:disabled{color:#CDD8E2;cursor:not-allowed;}
.tgs-cal-day.tgs-om{color:#DDE8F0;}
.tgs-slots{padding:8px 10px 8px;display:flex;flex-wrap:wrap;gap:7px;
  border-top:1px solid #E8EFF6;}
.tgs-slot{padding:6px 13px;border-radius:20px;border:1.5px solid #D0DDE8;
  background:#fff;font-size:12.5px;font-weight:600;color:#1A3A5C;cursor:pointer;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
  transition:border-color .12s,background .12s,color .12s,box-shadow .12s;}
.tgs-slot:hover{border-color:#003B70;background:#EAF2FC;color:#003B70;
  box-shadow:0 2px 6px rgba(0,59,112,.1);}
.tgs-slot.tgs-sel{background:#FFD23F;border-color:#FFD23F;color:#002B54;
  box-shadow:0 2px 8px rgba(255,210,63,.3);}
.tgs-slots-load{padding:14px;text-align:center;font-size:12px;color:#B0C2D2;font-style:italic;}
.tgs-slots-empty{padding:12px;font-size:12.5px;color:#B0C2D2;text-align:center;}
...
<div id="tgs-footer">Crafted by <a href="https://www.afa-ai.com/" target="_blank" rel="noopener noreferrer">AFA</a></div>`;
  ...
  function bookCalendar(){
    clearChips();
    showTyping();
    setTimeout(()=>{ hideTyping(); bot('Wählen Sie Ihren Wunschtermin:'); renderCal('booking'); }, 1050);
  }
...
<div class="tgs-ar"><span class="tgs-al">Firma</span><span class="tgs-avc">${esc(a.unternehmen||'–')}</span></div>`;
  ...
  function reschedStart(){
    ...
    showTyping();
    setTimeout(()=>{ hideTyping(); bot('Wählen Sie den neuen Wunschtermin:'); renderCal('reschedule'); }, 1050);
  }
    ...
    try{
      ...
      if(res&&res.success===false){
        ...
        if(['outside_business_hours','not_available','slot_unavailable','appointment_not_available'].includes(er)||res.chatbot_not_available_message){
          ...
          document.getElementById('tgs-conf-wrap')?.remove();
          acts([
            {label:'Anderen Termin', cls:'p', fn:()=>{
              S.cal.selDate=null; S.cal.selTime=null;
              const cw=document.getElementById('tgs-cal-wrap');
              if(cw) paintCal(cw,'reschedule'); clearChips();
            }},
  ...
  function resetCal(){
    const t=new Date();
    S.cal={year:t.getFullYear(), month:t.getMonth(), selDate:null, selTime:null, slots:[]};
  }

  function renderCal(mode){
    const wrap=document.createElement('div'); wrap.id='tgs-cal-wrap';
    appendEl(wrap); paintCal(wrap, mode);
  }

  function paintCal(wrap, mode){
    wrap.innerHTML='';
    const c=S.cal;
    const today=new Date(); today.setHours(0,0,0,0);
    const first=new Date(c.year,c.month,1);
    const last =new Date(c.year,c.month+1,0);
    const startOff=((first.getDay()+6)%7);
    const cells=Math.ceil((startOff+last.getDate())/7)*7;
    const prevDis=new Date(c.year,c.month-1,1)<new Date(today.getFullYear(),today.getMonth(),1);

    const cal=document.createElement('div'); cal.className='tgs-cal';
    cal.innerHTML=`
<div class="tgs-cal-hd">
  <button class="tgs-cal-nav" id="tgs-cprev" ${prevDis?'disabled':''}>‹</button>
  <span class="tgs-cal-mo">${MONTHS[c.month]} ${c.year}</span>
  <button class="tgs-cal-nav" id="tgs-cnext">›</button>
</div>
<div class="tgs-cal-grid" id="tgs-cgrid"></div>
<div id="tgs-cslots"></div>`;
    wrap.appendChild(cal);

    const grid=cal.querySelector('#tgs-cgrid');
    DOWS.forEach(d=>{ const h=document.createElement('div'); h.className='tgs-cal-dow'; h.textContent=d; grid.appendChild(h); });

    for(let i=0;i<cells;i++){
      const dn=i-startOff+1;
      const btn=document.createElement('button'); btn.className='tgs-cal-day';
      if(dn<1||dn>last.getDate()){
        btn.className+=' tgs-om';
        btn.textContent=dn<1?new Date(c.year,c.month,dn).getDate():(dn-last.getDate());
        btn.disabled=true;
      }else{
        btn.textContent=dn;
        ...
        if(isPast||isWe){ btn.disabled=true; }
        else{
          if(dt.toDateString()===today.toDateString()) btn.className+=' tgs-today';
          if(c.selDate===ds) btn.className+=' tgs-sel';
          btn.dataset.d=ds;
          btn.addEventListener('click',()=>{ c.selDate=ds; c.selTime=null; paintCal(wrap,mode); });
        }
      }
      grid.appendChild(btn);
    }

    cal.querySelector('#tgs-cprev').addEventListener('click',()=>{
      c.month--; if(c.month<0){c.month=11;c.year--;}
      c.selDate=null; c.selTime=null; c.slots=[]; paintCal(wrap,mode);
    });
    cal.querySelector('#tgs-cnext').addEventListener('click',()=>{
      c.month++; if(c.month>11){c.month=0;c.year++;}
      c.selDate=null; c.selTime=null; c.slots=[]; paintCal(wrap,mode);
    });

    if(c.selDate) loadSlots(cal.querySelector('#tgs-cslots'), c.selDate, mode);
    scroll();
  }

  async function loadSlots(area, dateStr, mode){
    area.innerHTML='<div class="tgs-slots-load">Verfügbare Zeiten werden geladen…</div>';
    scroll();
    try{
      ...
      S.cal.slots=slots;
      if(!slots.length){ area.innerHTML='<div class="tgs-slots-empty">Keine freien Termine an diesem Tag.</div>'; return; }
      area.innerHTML='';
      const sd=document.createElement('div'); sd.className='tgs-slots';
      slots.forEach(t=>{
        const b=document.createElement('button');
        b.className='tgs-slot'+(S.cal.selTime===t?' tgs-sel':'');
        b.textContent=t+' Uhr';
        b.addEventListener('click',()=>{
          S.cal.selTime=t;
          sd.querySelectorAll('.tgs-slot').forEach(x=>x.classList.remove('tgs-sel'));
          b.classList.add('tgs-sel');
          showConfirm(mode);
        });
        sd.appendChild(b);
      });
      ...
      area.innerHTML='<div class="tgs-slots-empty">Verfügbarkeit konnte nicht geladen werden.</div>';
    }
    scroll();
  }
  ...
  function showConfirm(mode){
    document.getElementById('tgs-conf-wrap')?.remove();
    const wrap=document.createElement('div'); wrap.id='tgs-conf-wrap';
    const card=document.createElement('div'); card.className='tgs-cc';
    card.innerHTML=`
<div class="tgs-cr"><span class="tgs-cl">Datum</span><span class="tgs-cv">${fmtDate(S.cal.selDate)}</span></div>
<div class="tgs-cr"><span class="tgs-cl">Uhrzeit</span><span class="tgs-cv">${S.cal.selTime} Uhr</span></div>
${mode==='booking'?`<div class="tgs-cr"><span class="tgs-cl">Thema</span><span class="tgs-cv">${esc(S.data.interesse||'')}</span></div>`:''}
${mode==='reschedule'?`<div class="tgs-cr"><span class="tgs-cl">Aktuell</span><span class="tgs-cv">${esc(S.appointment?.termin||'–')}</span></div>`:''}`;
    wrap.appendChild(card); appendEl(wrap);
    acts([
      {label:'Termin bestätigen', cls:'a', fn:()=>mode==='booking'?bookSubmit():reschedSubmit()},
      {label:'Datum ändern',      cls:'g', fn:()=>{
        S.cal.selDate=null; S.cal.selTime=null; wrap.remove();
        const cw=document.getElementById('tgs-cal-wrap'); if(cw) paintCal(cw,mode);
        clearChips(); scroll();
      }},
      {label:'Zeit ändern',       cls:'g', fn:()=>{
        S.cal.selTime=null; wrap.remove();
        const sa=document.querySelector('#tgs-cslots');
        if(sa&&S.cal.selDate) loadSlots(sa,S.cal.selDate,mode);
        clearChips(); scroll();
      }},
    ]);
  }
