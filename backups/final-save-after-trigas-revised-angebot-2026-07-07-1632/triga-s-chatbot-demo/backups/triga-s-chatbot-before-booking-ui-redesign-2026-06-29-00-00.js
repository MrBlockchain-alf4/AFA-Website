...
/* ─── BOOKING STAGE (unified persistent container) ── */
.tgs-book-stage{
  background:#fff;border-radius:14px;border:1px solid rgba(0,72,130,.14);
  overflow:hidden;flex-shrink:0;
  box-shadow:0 2px 20px rgba(0,38,80,.10),0 0 0 1px rgba(0,72,130,.04);
  animation:tgsMsgEnter .3s cubic-bezier(.22,1,.36,1) both;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}
/* Header */
.tgs-bk-hd{
  background:linear-gradient(135deg,#002C58 0%,#004B8A 100%);
...
.tgs-bk-nav-btn:hover:not(:disabled){background:rgba(255,255,255,.22);}
.tgs-bk-nav-btn:disabled{opacity:.18;cursor:not-allowed;}
.tgs-bk-dots{display:flex;gap:5px;align-items:center;}
.tgs-bk-dot{width:6px;height:6px;border-radius:50%;transition:background .2s,transform .2s;}
.tgs-bk-dot.done{background:rgba(255,255,255,.40);}
.tgs-bk-dot.active{background:#FFD23F;transform:scale(1.25);}
.tgs-bk-dot.todo{background:rgba(255,255,255,.16);}
.tgs-bk-hd-info{display:flex;flex-direction:column;gap:1px;}
.tgs-bk-hd-sub{color:rgba(255,255,255,.52);font-size:9.5px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;}
.tgs-bk-hd-main{color:#fff;font-size:13px;font-weight:700;letter-spacing:.01em;}
.tgs-bk-month{color:#fff;font-size:13px;font-weight:700;letter-spacing:.01em;}
/* Body */
.tgs-bk-body{padding:8px 10px 4px;}
/* 5-col weekday grid */
.tgs-bk-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:2px;}
.tgs-bk-dow{
  font-size:9px;font-weight:700;color:#8FA3B4;text-align:center;
  padding:1px 0 5px;text-transform:uppercase;letter-spacing:.06em;}
.tgs-bk-day{
  border-radius:6px;border:none;background:transparent;
  cursor:pointer;font-size:12px;font-weight:500;color:#1E3347;
  display:flex;align-items:center;justify-content:center;height:28px;
  transition:background .11s,color .11s,box-shadow .11s;}
.tgs-bk-day:hover:not(:disabled):not(.sel){background:#E3EEF9;color:#002C58;}
.tgs-bk-day.today{color:#004B8A;font-weight:800;box-shadow:inset 0 0 0 1px rgba(0,75,138,.28);background:#EAF3FC;}
.tgs-bk-day.sel{background:#002C58;color:#fff;font-weight:700;box-shadow:0 2px 8px rgba(0,38,80,.22);}
.tgs-bk-day.past{color:#CBD8E5!important;cursor:not-allowed;}
.tgs-bk-day.blank{background:transparent;cursor:default;pointer-events:none;}
.tgs-bk-day:disabled{color:#CBD8E5;cursor:not-allowed;}
/* Slots area */
.tgs-bk-slots-area{
  padding:8px 10px 4px;max-height:174px;overflow-y:auto;
  scrollbar-width:thin;scrollbar-color:#C4D4E4 transparent;}
.tgs-bk-slot-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;}
.tgs-bk-slot{
  padding:7px 2px;border-radius:7px;border:1px solid #CDDAE6;
  background:#fff;font-size:11px;font-weight:600;color:#1A3A5C;
  cursor:pointer;text-align:center;
  transition:border-color .11s,background .11s,color .11s;}
.tgs-bk-slot:hover{border-color:#004B8A;background:#EAF3FC;color:#002C58;}
.tgs-bk-slot.sel{background:#FFD23F;border-color:#D9A800;color:#1A1800;box-shadow:0 2px 6px rgba(255,210,63,.26);}
.tgs-bk-slot-msg{padding:18px 8px;text-align:center;font-size:12px;color:#96B0C5;font-style:italic;}
/* Footer */
.tgs-bk-footer{padding:6px 10px 9px;display:flex;flex-direction:column;gap:5px;border-top:1px solid rgba(0,72,130,.06);}
...
.tgs-bk-btn.pri{background:#FFD23F;color:#14190A;box-shadow:0 3px 10px rgba(255,210,63,.26);}
.tgs-bk-btn.pri:hover:not(:disabled){background:#F5C800;transform:translateY(-1px);}
.tgs-bk-btn.pri:disabled{opacity:.30;cursor:not-allowed;box-shadow:none;transform:none;}
.tgs-bk-btn.sec{background:transparent;color:#547088;border:1px solid #C0D0DE;}
.tgs-bk-btn.sec:hover:not(:disabled){background:#EAF3FC;border-color:#7AAACE;color:#1A3A5C;}
/* Confirm body */
.tgs-bk-conf-body{padding:10px 12px 6px;}
.tgs-bk-conf-row{
  display:flex;justify-content:space-between;align-items:baseline;
  gap:8px;padding:3px 0;font-size:12.5px;border-bottom:1px solid rgba(0,72,130,.05);}
.tgs-bk-conf-row:last-child{border-bottom:none;}
.tgs-bk-sec-lbl{color:#547088;font-weight:500;white-space:nowrap;flex-shrink:0;}
.tgs-bk-sec-val{color:#16253A;font-weight:600;text-align:right;word-break:break-word;}
.tgs-bk-conf-sep{border:none;height:1px;background:rgba(0,72,130,.09);margin:6px 0;}
.tgs-bk-conf-footer{padding:6px 10px 9px;display:flex;flex-direction:column;gap:5px;border-top:1px solid rgba(0,72,130,.06);}
...
<div class="tgs-ar"><span class="tgs-al">Firma</span><span class="tgs-avc">${esc(a.unternehmen||'–')}</span></div>`;
  ...
  function paintDateStep(mode){
    ...
    weitBtn.textContent='Weiter zur Uhrzeit →'; weitBtn.disabled=!c.selDate;
  ...
  function showTimeStep(mode){
    ...
    setTimeout(()=>{
      hideTyping();
      bot('Bitte wählen Sie Ihre bevorzugte Uhrzeit:');
      paintTimeStep(mode);
    }, 900);
  }
  ...
  function showConfirm(mode){
    ...
    setTimeout(()=>{
      ...
      uhrzeitBtn.textContent='← Uhrzeit';
