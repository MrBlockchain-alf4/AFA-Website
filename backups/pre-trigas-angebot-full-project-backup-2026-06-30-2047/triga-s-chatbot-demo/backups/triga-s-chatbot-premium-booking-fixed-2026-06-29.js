...
.tgs-bk-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:4px;}
.tgs-bk-dow{
...
/* ─── Time slots area ── */
.tgs-bk-slots-area{flex:1;overflow:hidden;padding:10px 12px 6px;}
.tgs-bk-slot-grid{
  display:grid;grid-template-columns:repeat(3,1fr);
  gap:5px;width:100%;box-sizing:border-box;
}
.tgs-bk-slot{
  height:30px;border-radius:6px;border:1px solid #D6E3F0;background:#fff;
...
/* ─── Confirm body ── */
.tgs-bk-conf-body{flex:1;overflow:hidden;padding:9px 13px 4px;}
.tgs-bk-conf-section{margin-bottom:7px;}
.tgs-bk-conf-section-label{
  font-size:8px;font-weight:700;color:#6A8AAA;
  text-transform:uppercase;letter-spacing:.12em;
  margin-bottom:3px;padding-bottom:3px;
  border-bottom:1px solid rgba(0,30,80,.07);
}
.tgs-bk-conf-row{
...
.tgs-bk-sec-lbl{
  ...
  flex-shrink:0;font-size:10.5px;min-width:52px;
}
.tgs-bk-sec-val{
  color:#09192C;font-weight:600;text-align:right;
  ...
  function showConfirm(mode){
    ...
    setTimeout(()=>{
      ...
      if(mode==='booking'){
        ...
        if(nm) contSec.appendChild(mkRow('Name',esc(nm)));
        if(S.data.email) contSec.appendChild(mkRow('E-Mail',esc(S.data.email)));
        contSec.appendChild(mkRow('Telefon',S.data.telefon?esc(S.data.telefon):'nicht angegeben'));
        contSec.appendChild(mkRow('Firma',S.data.unternehmen?esc(S.data.unternehmen):'nicht angegeben'));
        if(S.data.interesse) contSec.appendChild(mkRow('Anliegen',esc(S.data.interesse)));
