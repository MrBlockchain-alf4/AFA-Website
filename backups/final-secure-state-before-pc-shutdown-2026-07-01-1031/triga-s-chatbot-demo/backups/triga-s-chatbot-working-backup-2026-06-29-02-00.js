...
/* ─── BOOKING STAGE — TRIGA-S lab scheduler ─────── */
.tgs-book-stage{
  background:#fff;border-radius:10px;
  border:1px solid #B6C8DC;
  overflow:hidden;flex-shrink:0;
  box-shadow:0 2px 6px rgba(0,22,52,.08),0 8px 26px rgba(0,22,52,.09);
  animation:tgsMsgEnter .3s cubic-bezier(.22,1,.36,1) both;
  font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;
  display:flex;flex-direction:column;height:320px;}
/* ─── Header strip ── */
.tgs-bk-hd{
  background:linear-gradient(135deg,#001628 0%,#002C56 100%);
  padding:9px 13px 7px;flex-shrink:0;
  border-bottom:2px solid rgba(255,210,63,.30);}
...
.tgs-bk-nav-btn:hover:not(:disabled){background:rgba(255,255,255,.14);}
.tgs-bk-nav-btn:disabled{opacity:.14;cursor:not-allowed;}
/* Step progress bars */
...
.tgs-bk-dot.done{background:rgba(255,210,63,.40);width:16px;}
.tgs-bk-dot.active{background:#FFD23F;width:20px;}
.tgs-bk-dot.todo{background:rgba(255,255,255,.11);width:16px;}
/* Header info block (time+confirm steps) */
.tgs-bk-hd-info{display:flex;flex-direction:column;gap:1px;}
.tgs-bk-hd-sub{color:rgba(255,255,255,.34);font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.10em;}
.tgs-bk-hd-main{color:#fff;font-size:12px;font-weight:700;letter-spacing:.01em;}
/* Month label (calendar step) */
.tgs-bk-month{color:#fff;font-size:12px;font-weight:700;letter-spacing:.01em;}
/* ─── Calendar body — fills flex space ── */
.tgs-bk-body{flex:1;overflow:hidden;padding:8px 12px 4px;}
/* 5-col weekday grid */
.tgs-bk-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:3px;}
.tgs-bk-dow{
  font-size:8px;font-weight:700;color:#6080A2;text-align:center;
  padding:0 0 5px;text-transform:uppercase;letter-spacing:.07em;}
/* Date cells */
.tgs-bk-day{
  border-radius:5px;border:1px solid #D0DCE8;background:#fff;
  cursor:pointer;font-size:12px;font-weight:400;color:#1C3450;
...
.tgs-bk-day:hover:not(:disabled):not(.sel):not(.past):not(.blank){
  background:#EBF3FB;border-color:#5090C8;}
.tgs-bk-day.today{
  background:#E2F0FF;border-color:#3A7EC0;color:#002050;font-weight:600;}
.tgs-bk-day.sel{
  background:#002C58;color:#fff;font-weight:700;
  border:1px solid #001E40;border-top:2px solid #FFD23F;
  box-shadow:0 2px 6px rgba(0,36,84,.20);}
.tgs-bk-day.past{
  background:transparent;border-color:#ECF0F4;color:#C4CDD6;cursor:not-allowed;}
.tgs-bk-day.blank{
  background:transparent;border:none;cursor:default;pointer-events:none;}
.tgs-bk-day:disabled:not(.past){
  background:#F7F9FB;border-color:#E8EEF4;color:#BECAD2;cursor:not-allowed;}
/* ─── Time slots area — fills flex space ── */
.tgs-bk-slots-area{flex:1;overflow:hidden;padding:8px 12px 4px;}
/* 4-column grid — stable, no clip */
.tgs-bk-slot-grid{
  display:grid;grid-template-columns:repeat(4,1fr);
  gap:5px;width:100%;box-sizing:border-box;}
.tgs-bk-slot{
  height:28px;border-radius:5px;border:1px solid #D0DCE8;
  background:#fff;font-size:12px;font-weight:500;color:#1C3450;
  cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  transition:background .1s,border-color .1s,color .1s;
  white-space:nowrap;overflow:hidden;
  font-variant-numeric:tabular-nums;}
.tgs-bk-slot:hover:not(.sel){background:#EBF3FB;border-color:#5090C8;}
.tgs-bk-slot.sel{
  background:#002C58;color:#fff;
  border:1px solid #001E40;border-top:2px solid #FFD23F;
  box-shadow:0 1px 4px rgba(0,36,84,.18);}
.tgs-bk-slot-msg{
  padding:10px 8px;text-align:center;font-size:11.5px;color:#8FA5B8;font-style:italic;}
/* ─── Footer — shared, fixed size ── */
.tgs-bk-footer{
  flex-shrink:0;padding:6px 12px 9px;
...
.tgs-bk-btn.pri{
  background:#FFD23F;color:#181400;
  box-shadow:0 2px 8px rgba(255,210,63,.28);}
.tgs-bk-btn.pri:hover:not(:disabled){background:#F5C800;box-shadow:0 4px 12px rgba(255,210,63,.36);}
...
.tgs-bk-btn.sec{
  background:#EFF4F9;color:#2C4460;
  border:1px solid #C6D4E2;}
.tgs-bk-btn.sec:hover:not(:disabled){background:#E3EDF8;border-color:#8AB0D0;color:#162D42;}
.tgs-bk-btn.sec:active:not(:disabled){background:#D6E8F4;}
/* ─── Confirm data table — fills flex space ── */
.tgs-bk-conf-body{flex:1;overflow:hidden;padding:8px 12px 3px;}
.tgs-bk-conf-row{
  display:flex;justify-content:space-between;align-items:baseline;
  gap:8px;padding:4px 0;font-size:12px;
  border-bottom:1px solid rgba(0,40,90,.06);}
.tgs-bk-conf-row:last-child{border-bottom:none;}
.tgs-bk-sec-lbl{color:#5C7898;font-weight:500;white-space:nowrap;flex-shrink:0;font-size:11px;}
.tgs-bk-sec-val{color:#0B1C2E;font-weight:600;text-align:right;word-break:break-word;font-size:12px;}
.tgs-bk-conf-sep{border:none;height:1px;background:rgba(0,40,90,.07);margin:3px 0;}
/* ─── Confirm footer — fixed size ── */
.tgs-bk-conf-footer{
  flex-shrink:0;padding:6px 12px 9px;
  display:flex;flex-direction:column;gap:5px;
  border-top:1px solid rgba(0,40,90,.08);}
  ...
  function showConfirm(mode){
    ...
    setTimeout(()=>{
      ...
      if(mode==='booking'){
        ...
        dataBtn.textContent='Kontakt ändern';
